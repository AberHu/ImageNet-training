...
def report_format(collected_nodes, brief_report=False):
	data = list()
	...
	df = df.fillna(' ')
	df['params'] = df['params'].apply(lambda x: '{:,}'.format(x))
	...
	df['MemRead(B)'] = df['MemRead(B)'].apply(lambda x: '{:,}'.format(x))
	df['MemWrite(B)'] = df['MemWrite(B)'].apply(lambda x: '{:,}'.format(x))
	df['MemR+W(B)'] = df['MemR+W(B)'].apply(lambda x: '{:,}'.format(x))
