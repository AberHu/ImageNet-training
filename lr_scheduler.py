...
restart_lr_period = [10, 20, 40, 80, 160, 320]
restart_lr_step = [0, 10, 30, 70, 150, 310]

def get_lr_scheduler(args, optimizer, epoch_steps):
	max_steps = epoch_steps * args.epochs

	period_steps = [epoch_steps * x for x in restart_lr_period]
	step_steps = [epoch_steps * x for x in restart_lr_step]

	use_warmup = args.warmup
	if use_warmup:
		warmup_steps = args.warmup_epochs * epoch_steps
		warmup_startlr =args.warmup_ilr
		warmup_targetlr = args.warmup_tlr
	else:
		warmup_steps = 0
		warmup_startlr = args.ilr
		warmup_targetlr = args.ilr
	...
	scheduler = CosineRestartAnnealingLR(optimizer,
									...
									use_restart=use_cosine_restart)
